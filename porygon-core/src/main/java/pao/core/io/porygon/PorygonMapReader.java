...
public class PorygonMapReader implements MapReader {
    ...
    @Override
    public Map read(Path path) throws PorygonIOException {
        if (path == null) {
            throw new PorygonIOException("Specified path is null!");
        }

        if (!Files.exists(path)) {
            throw new PorygonIOException(String.format("The map file does not exist: %s", path));
        }
        ...
        try {
            ...
            throw new PorygonIOException(String.format("Could not read property from map file: %s\n%s", path, e.getMessage()));
        } catch (IOException e) {
            throw new PorygonIOException(String.format("Could not read map file: %s", path));
        }
    }
}
